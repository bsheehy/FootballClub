...
/*
 * =============================================
 * Addres Search Methods
 * =============================================
 */
function openAddressSearchResults(e) {
  ...
  if ($('#AddressTown').val()) {
    address += " " + $('#AddressTown').val();
  }
  if ($('#AddressPostCode').val()) {
    address += " " + $('#AddressPostCode').val();
  }
  if ($('#AddressCounty').val()) {
    address += " " + $('#AddressCounty').val();
  }
  if ($('#AddressCounty').val()) {
    addressCountry = $('#AddressCounty').val();
  }

  //string address, string country
  var addressActionUrlParams = { "address": address, "country": addressCountry };

  kendoDialogCreate(dialogGuid, closejscallback, dialogToFit);
  kendoDialogOpen(dialogGuid, dialogTitle, addressActionUrl, addressActionUrlParams);
}

function addressSearchGetParams(searchCriteriaDiv) {
  var data = {};
  if (!searchCriteriaDiv) {
    searchCriteriaDiv = "#searchCriteria";
  }
...
function selectPersonAddress(e) {
  var row = $(e.currentTarget).closest("tr");
  var gridId = this.element.attr("id");
  var dataItem = this.dataItem(row);

  if(dataItem.AddressNumber)
  {
    $('#AddressNumber').val(dataItem.AddressNumber);
  }
  if (dataItem.AddressStreet) {
    $('#AddressStreet').val(dataItem.AddressStreet);
  }
  if (dataItem.AddressTown) {
    $('#AddressTown').val(dataItem.AddressTown);
  }
  if (dataItem.AddressPostCode) {
    $('#AddressPostCode').val(dataItem.AddressPostCode);
  }
  if (dataItem.AddressXlngCoord) {
    $('#AddressXlngCoord').val(dataItem.AddressXlngCoord);
  }
  if (dataItem.AddressYlatCoord) {
    $('#AddressYlatCoord').val(dataItem.AddressYlatCoord);
  }
  if (dataItem.AddressCountry) {
    $('#AddressCountry').val(dataItem.AddressCountry);
  }
